...
while (<FAM>) {
	...
	#$cmd="unzip -p /mnt/isilon/cag_ngs/hiseq/glessner/ABCD_Array_ForCNVCalling_Aaron/genomics_sample03/CNV/ABCD_Array_ForCNVCalling_BAF_LRR_int.csv_x11088.zip $ind[$i]>temp/$ind[$i]";
	#@out=`$cmd`;
	
		my $SampleIDInFileHeader = $ind[$i];
		$SampleIDInFileHeader =~ s/.int.csv//;
		$SAMPLEID = $SampleIDInFileHeader; #For QuantiSNP environment variable

		$INFILE = "$ind[$i]"; #For QuantiSNP environment variable
=pod	
...
"ABCD_Affymetrix_SmokeScreen_BATCH_2_WB.sample",664231,
    ...
    foreach my $key (keys %platver) {
        if($CountPath[0]-1 == $platver{$key}) {
            ...
            #`$command`;
	    #was $command = "awk -F, '{print \$2,\$1}' temp/$ind[$i] | paste NameChrPos.txt - | perl -pe 's/,/\\t/' | sed 's/\\r//' | grep -v REMOVE >> $ind[$i]";
            #$command = "cat temp/$ind[$i] | paste NameChrPos.txt - | perl -pe 's/,/\\t/' | sed 's/\\r//' | grep -v REMOVE >> $ind[$i]";
            #`$command`;
	$fg=$key;	
        }
    }
		...
		#Just run once at beginning in slurm bash script
		#$command = "/bin/bash -c \"module load singularity/3.7\"";
		#my @output=`$command`;
		#print "@output\n";

		$command = "sed \"s#SAMPLEID=AB0000070_NDAR_INVF7UTX830.int.csv_LrrBaf#SAMPLEID=$SAMPLEID#\" /mnt/isilon/cag_ngs/hiseq/glessner/www.stats.ox.ac.uk/pub/yau/quantisnp2/QuantiSNP_Singularity_env-file_wExport.txt | sed \"s#INFILE=/mnt/isilon/cag_ngs/hiseq/glessner/ABCD_Array_ForCNVCalling_Aaron/genomics_sample03/CNV/AB0000070_NDAR_INVF7UTX830.int.csv_LrrBaf#INFILE=$INFILE#\" > QuantiSNP_Singularity_env-file_wExport_temp.txt";
		@output=`$command`;
		
		if(!($noPennCNV))
		{
		$command = 'singularity run -B /mnt/isilon/cag_ngs/hiseq/glessner/ABCD_Array_ForCNVCalling_Aaron/genomics_sample03/CNV/ --env-file QuantiSNP_Singularity_env-file_wExport_temp.txt /mnt/isilon/cag_ngs/hiseq/glessner/www.stats.ox.ac.uk/pub/yau/quantisnp2/quantisnp_samesense_quantisnp-matlab.sif /usr/local/bin/quantisnp/quantisnp/linux64/./run_quantisnp2.sh \$MCRROOT --chr \$CHRRANGE --outdir \$OUTDIR --sampleid \$SAMPLEID --emiters \$EMITERS --lsetting \$LSETTING --gcdir \$GCDIR --config \$PARAMSFILE --levels \$LEVELSFILE --input-files \$INFILE --chrX \$CHRX --doXcorrect';             #BUILD SPECIFICATION AND SNP SET TO INCLUDE IN PFB
		
		print LOG "running $command\n";
		my @output=`$command`;
		if($chrx)
		{
		$command = "perl /cm/shared/easybuild/software/PennCNV/1.0.5-GCCcore-8.3.0/detect_cnv.pl -test -hmm /cm/shared/easybuild/software/PennCNV/1.0.5-GCCcore-8.3.0/lib/hhall.hmm -pfb $PFB_FILE --gcmodel /mnt/isilon/cag_ngs/hiseq/glessner/mnt/public/SNPMAP_FILES/GC\_$build/$fg\_$build.gcmodel --chrx --conf $triofile[$i] >>myCall.rawcnv 2>>$ARGV[0].Intersect.log";             #BUILD SPECIFICATION AND SNP SET TO INCLUDE IN PFB
		print LOG "running $command\n";
		my @output=`$command`;
		...
		$command = "perl /cm/shared/easybuild/software/PennCNV/1.0.5-GCCcore-8.3.0/detect_cnv.pl -test -hmm /cm/shared/easybuild/software/PennCNV/1.0.5-GCCcore-8.3.0/lib/hhall.hmm -pfb myYasX.pfb --gcmodel /mnt/isilon/cag_ngs/hiseq/glessner/mnt/public/SNPMAP_FILES/GC\_$build/$fg\_$build.gcmodel --chrx --conf $triofile[$i] >myYasXCall.rawcnv 2>myYasXCall.log";
		print LOG "running $command\n";
                @output=`$command`;
		...
		#print LOG "running $command\n";
                #@output=`$command`;
		$command = "perl /cm/shared/easybuild/software/PennCNV/1.0.5-GCCcore-8.3.0/clean_cnv.pl combineseg myCall.rawcnv --signalfile $triofile[$i] >> $ARGV[0].Intersect_Clean_CNV.rawcnv 2>>$ARGV[0].Intersect.log";
		print LOG "running $command\n";
		@output=`$command`;
		}
		...
		if($loh)
		{
		$command = "perl /cm/shared/easybuild/software/PennCNV/1.0.5-GCCcore-8.3.0/detect_cnv.pl -test -hmm /cm/shared/easybuild/software/PennCNV/1.0.5-GCCcore-8.3.0/lib/hh550.hmm -pfb $PFB_FILE --gcmodel /mnt/isilon/cag_ngs/hiseq/glessner/mnt/public/SNPMAP_FILES/GC\_$build/$fg\_$build.gcmodel --loh --conf $triofile[$i] >>myCall.rawcnv 2>>$ARGV[0].Intersect.log";
                print LOG "running $command\n";
                @output=`$command`;
		}
		...
		if($GCAdjustOut)
		{
		$command = "perl /cm/shared/easybuild/software/PennCNV/1.0.5-GCCcore-8.3.0/genomic_wave.pl -adjust -gcmodel /mnt/isilon/cag_ngs/hiseq/glessner/mnt/public/SNPMAP_FILES/GC\_$build/$fg\_$build.gcmodel $triofile[$i]";
		print LOG "running $command\n";
		@output=`$command`;
		print @output;
		}
		...
		print LOG "running $command\n";
                @output=`$command`;
		$command = "perl /cm/shared/easybuild/software/PennCNV/1.0.5-GCCcore-8.3.0/filter_cnv.pl $ARGV[0].Intersect_Clean_CNV.rawcnv_wPath -qclogfile $ARGV[0].Intersect.log_wPath -qcsumout QCsum.qcsum -out goodCNV.good.cnv -chroms 1-22";
		print LOG "running $command\n";
                @output=`$command`;
